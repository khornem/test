...
# v0.6  15-Mar-2014
# -----------------
# - changed script to use my format of NETMAP, where first router is a TAP router
#   second router is an sniffer router and subsequent routers are normal routers
#
# v0.5, 20-Sep-2011
# -----------------
...
-p instance ID
    ...
    Modified and this new format is mandatory:
    
    8xx:0/0 9xx:0/0 <rid>:x/y
    
    where 8xx will be used as tap router and 9xx may be used to
    make a sniffer capture
...
while (<netmap_handle>) {
    ...
    next
#original, lo cambio para que utilice mi formato de NETMAP:
# 800:0/0 900:0/0 RID:...
#      if !( $_ =~
#        m/^\d+:\d+\/\d+@[\w-]+[ \t]+$pseudo_instance:\d+\/\d+@[\w-]+(\s|\t)*$/
#      );
      if !( $_ =~
        m/^$pseudo_instance:\d+\/\d+/
      );
    my $inputline = $_;
    chomp($inputline);
...
#divide la linea por router 
    my @connline = split( /[ \t]+/, $inputline );
#router 800
    $connline[0] =~ s/(\s\t)*//g; 
#router 900
    $connline[1] =~ s/(\s\t)*//g;
#router real en cualquier otro rango
    $connline[2] =~ s/(\s\t)*//g;
    my @iou_src = split( /:/, $connline[2] );
    my @iou_dst = split( /:/, $connline[0] );
